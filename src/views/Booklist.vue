...
export default {
  ...
  methods:{ 
    putOnBook(){
      ...
      // 提交到后端
      axios.post('http://127.0.0.1:8000/books', this.newBook)
        .then(response => {
    ...
    cutOutBook() {
      ...
      // 从后端删除
      axios.delete(`http://127.0.0.1:8000/books/${this.idToDelete}`)
        .then(response => {
          console.log(response.data.message);
        })
        .catch(error => {
    ...
    saveChanges(){
      ...
      if (index != -1) {
        ...
        // 后端更改数据
        axios.put(`http://127.0.0.1:8000/books/${this.idOfselectedBook}`, this.bookList[index])
          .then(response=>{
            console.log(response.data.message);
          })
          .catch(error=>{
